...
# Conexión
server = 'LAPTOP-6A0QS6G2\\SQLEXPRESS'
database = 'IMSS_RESUMEN'
conn_str = f'mssql+pyodbc://@{server}/{database}?driver=ODBC+Driver+17+for+SQL+Server'
engine = create_engine(conn_str)

query = """
SELECT cat_region_municipio_imss_inegi.nombre_municipio, 
       SUM(asg_resumen.asegurados) AS asegurados, 
       asg_resumen.fecha
FROM asg_resumen 
LEFT OUTER JOIN cat_region_municipio_imss_inegi 
    ON asg_resumen.cve_municipio = cat_region_municipio_imss_inegi.cve_municipio_imss
WHERE cat_region_municipio_imss_inegi.nombre_municipio = 'Guadalajara'
GROUP BY cat_region_municipio_imss_inegi.nombre_municipio, asg_resumen.fecha
HAVING asg_resumen.fecha < CONVERT(DATETIME, '2025-04-01 00:00:00', 102)
ORDER BY asg_resumen.fecha
"""
